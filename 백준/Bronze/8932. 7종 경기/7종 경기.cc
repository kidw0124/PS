...
multiset<ll>as,Bs,cs,ds;

bool pos(ll n, bool b){
    if(n==0)return true;
    bitset<64>BBB(n);
    if(BBB.count()==1&&b){
        ds.insert(__lg(n));
    ...
    for(ll i=0;i<63;i++){
        ...
        if(bs.count()==1){
            if(b){
                as.insert(i);
                Bs.insert(__lg(bs.to_ullong()));
            }
            else{
                Bs.insert(i);
                as.insert(__lg(bs.to_ullong()));
            }
            return true;
        }
    }
...
void solve(ll testcase) {
    ...
    for(i=0;i<7;i++){
        sum+=floor(cons[i][0]*pow(abs(cons[i][1]-arr[i]),cons[i][2]));
    }
    cout<<sum<<'\n';
