...
struct irrational{
    ll a,b,c,d;
    irrational(ll a, ll b, ll c, ll d):a(a),b(b),c(c),d(d){
        this->normalize(); // this-> 는 생략 가능
    }
    ...
    void normalize(){
        if(a<0)a=-a, b=-b, c=-c;
        if(c==0)d=0;
        ll g = gcd(gcd(a,abs(b)),abs(c));
        a/=g, b/=g, c/=g;
    }
    friend istream& operator>>(istream &is, irrational &r);
    friend ostream& operator<<(ostream &os, const irrational &r);
};
...
istream& operator>>(istream &is, irrational &r){
    r.input(is);
    return is;
}

ostream& operator<<(ostream &os, const irrational &r){
    r.output(os);
    return os;
}
