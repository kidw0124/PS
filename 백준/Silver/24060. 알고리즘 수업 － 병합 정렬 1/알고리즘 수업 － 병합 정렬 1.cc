...
void merge(vector<ll>&arr,ll p, ll q, ll r, ll&cnt, ll&o){
    ll i=p,j=q+1;
    vector<ll>tmp;
    while(i<=q && j<=r){
        if(arr[i]<=arr[j]){
...
void merge_sort(vector<ll>&arr,ll p,ll r,ll&o,ll&cnt){
    if(p<r){
        ll q=(p+r)>>1;
        merge_sort(arr,p,q,o,cnt);
        merge_sort(arr,q+1,r,o,cnt);
        merge(arr,p,q,r,cnt,o);
    }
}
