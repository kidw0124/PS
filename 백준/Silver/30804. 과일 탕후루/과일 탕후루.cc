...
struct Fenwick {
    const ll MAXN = 200'000;
    vector<ll> tree;
    Fenwick(ll sz) : tree(sz + 1) {}
    Fenwick() : Fenwick(MAXN+10) {}
    ...
    void add(ll p, ll val) {
        for (; p < tree.size(); p += p & -p) tree[p] += val;
    }
};
...
void solve(ll testcase){
    ...
    for(i=1;i<=9;i++){
        for(j=i+1;j<=9;j++){
            vector<ll>brr;
            brr.push_back(-1);
            for(m=0;m<n;m++)if(arr[m]!=i&&arr[m]!=j)brr.push_back(m);
            brr.push_back(n);
            for(m=1;m<brr.size();m++){
                if(brr[m]-brr[m-1]>ans)ans=brr[m]-brr[m-1];
            }
        }
    }
    cout<<ans-1<<"\n";
