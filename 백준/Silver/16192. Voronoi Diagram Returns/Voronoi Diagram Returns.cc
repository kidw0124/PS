...
void solve(){
    ...
    for(i=0;i<q;i++){
        ...
        vector<ll>brr(n),crr;
        for(j=0;j<n;j++){
            brr[j]=(x-arr[j].first)*(x-arr[j].first);
            brr[j]+=(y-arr[j].second)*(y-arr[j].second);
        }
        crr=brr;
        reverse(all(crr));
        ll m=*min_element(all(brr));
        ll c=count(all(brr),m);
        if(c>=3){
            cout<<"POINT\n";
        }
        else if(c==2){
            cout<<"LINE "<<find(all(brr),m)-brr.begin()+1<<' '<<crr.end()-find(all(crr),m)<<'\n';
        }
        else{
            cout<<"REGION "<<find(all(brr),m)-brr.begin()+1<<'\n';
        }
    }
