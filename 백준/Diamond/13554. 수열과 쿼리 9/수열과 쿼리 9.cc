...
void solve(ll testcase){
    ...
    vector<query>qu(m);
    for(i=0;i<m;i++){
        cin>>qu[i].s>>qu[i].e>>qu[i].k;
        qu[i].s--,qu[i].e--;
        qu[i].idx=i;
    }
    ll sn = sqrt(n);
    sort(qu.begin(),qu.end(),[&](const query& a,const query&b){
        ll sa = a.s/sn,sb=b.s/sn;
        return sa==sb?((sa&1)?a.e>b.e:a.e<b.e):sa<sb;
    });
    ll st=qu[0].s,en=qu[0].e;
    ll maxv=1e5;
    fenwick cnta(maxv),cntb(maxv);
    vector<ll>cnt_a(maxv+1),cnt_b(maxv+1);
    for(i=st;i<=en;i++){
        cnta.update(arr[i],1);
        cntb.update(brr[i],1);
        cnt_a[arr[i]]++;
    ...
    for(i=1;i<m;i++){
        while(en<qu[i].e){
            ...
            cnt_a[arr[idx]]++;
            cnt_b[brr[idx]]++;
            cnta.update(arr[idx],1);
            cntb.update(brr[idx],1);
        }
        while(st>qu[i].s){
            ll idx=--st;
            cnt_a[arr[idx]]++;
        ...
        ans=0;
        ll sk=min(maxv,(ll)sqrt((ld)(qu[i].k+eps)));
        for(j=1;j<=sk;j++){
            ans+=cnt_a[j]*cntb.sum(1,qu[i].k/j);
            ans+=cnt_b[j]*cnta.sum(1,qu[i].k/j);
        }
        ans-=cnta.sum(1,sk)*cntb.sum(1,sk);
        qu[i].ans=ans;
    }
    sort(qu.begin(),qu.end(),[&](const query& a,const query&b){
        return a.idx<b.idx;
