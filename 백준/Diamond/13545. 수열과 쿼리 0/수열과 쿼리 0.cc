...
void solve(ll testcase){
    ...
    ll n,m,r;
    cin>>n;
    vector<ll>tarr(n),arr(n+1);
    vector<deque<ll>>brr(2e5+1);
    vector<ll>drr(n+2);
    for(i=0;i<n;i++) cin>>tarr[i];
    arr[0]=1e5;
    for(i=1;i<=n;i++) arr[i]=arr[i-1]+tarr[i-1];
    n=n+1;
    cin>>m;
    vector<query>q(m);
    for(i=0;i<m;i++) {
    ...
    for(i=st;i<=en;i++){
        if(brr[arr[i]].size()>1){
            ll bef = brr[arr[i]].back()-brr[arr[i]].front();
            drr[bef]--;
            brr[arr[i]].push_back(i);
            ll aft = brr[arr[i]].back()-brr[arr[i]].front();
            drr[aft]++;
            if(ans<aft)ans=aft;
        }
        else if(brr[arr[i]].size()==1){
            brr[arr[i]].push_back(i);
            ll aft = brr[arr[i]].back()-brr[arr[i]].front();
            drr[aft]++;
            if(ans<aft)ans=aft;
    ...
    for(i=0;i<m;i++){
        while(en<q[i].e){
            ...
            else if(brr[x].size()==1){
                brr[x].push_back(en);
                ll aft = brr[x].back()-brr[x].front();
                drr[aft]++;
                if(ans<aft)ans=aft;
            }
            else{
                ll bef = brr[x].back()-brr[x].front();
                drr[bef]--;
                brr[x].push_back(en);
                ll aft = brr[x].back()-brr[x].front();
                drr[aft]++;
