...
void solve(ll testcase){
    ...
    ll n,m,r,l;
    cin>>n>>l;
    vector<int>tarr(n),arr(n+1);
    vector<int>rev(l+1,-1);
    vector<int>drr(n+2);
    vector<deque<int>>brr;
    for(i=0;i<n;i++) cin>>tarr[i],tarr[i]%=l;
    arr[0]=0;
    rev[0]=0;
    brr.push_back({});
    for(i=1;i<=n;i++) {
        arr[i]=(arr[i-1]+tarr[i-1])%l;
        if(rev[arr[i]]==-1)rev[arr[i]]=brr.size(),brr.push_back({});
    }
    n=n+1;
    cin>>m;
    vector<query>q(m);
    ...
    sort(q.begin(),q.end(),[&](const query& a,const query&b){
        ll sa = a.s/sn,sb=b.s/sn;
        return sa==sb?a.e<b.e:sa<sb;
    });
    ll st=q[0].s,en=q[0].e;
    ll ans=0;
    for(i=st;i<=en;i++){
        if(brr[rev[arr[i]]].size()>1){
            ll bef = brr[rev[arr[i]]].back()-brr[rev[arr[i]]].front();
            drr[bef]--;
            brr[rev[arr[i]]].push_back(i);
            ll aft = brr[rev[arr[i]]].back()-brr[rev[arr[i]]].front();
            drr[aft]++;
            if(ans<aft)ans=aft;
        }
        else if(brr[rev[arr[i]]].size()==1){
            brr[rev[arr[i]]].push_back(i);
            ll aft = brr[rev[arr[i]]].back()-brr[rev[arr[i]]].front();
            drr[aft]++;
            if(ans<aft)ans=aft;
    ...
    for(i=0;i<m;i++){
        while(en<q[i].e){
            ...
            ll x=arr[en];
            if(brr[rev[x]].size()==0){
                brr[rev[x]].push_back(en);
            }
            else if(brr[rev[x]].size()==1){
                brr[rev[x]].push_back(en);
                ll aft = brr[rev[x]].back()-brr[rev[x]].front();
                drr[aft]++;
                if(ans<aft)ans=aft;
            }
            else{
                ll bef = brr[rev[x]].back()-brr[rev[x]].front();
                drr[bef]--;
                brr[rev[x]].push_back(en);
                ll aft = brr[rev[x]].back()-brr[rev[x]].front();
                drr[aft]++;
        ...
        while(en>q[i].e){
            ...
            else if(brr[rev[x]].size()==2){
                ll bef = brr[rev[x]].back()-brr[rev[x]].front();
                drr[bef]--;
                if(ans==bef && drr[bef]==0){
                    while(drr[ans]==0&& ans > 0)ans--;
                }
                brr[rev[x]].pop_back();
            }
            else{
