...
void solve(ll testcase) {
    ...
    cin >> n;
    vector<ll> pentagon;
    for(i = 1; pentagon.empty() || pentagon.back()<n+1 ; i++) {
        pentagon.push_back(i * (3 * i - 1) / 2);
        pentagon.push_back(i * (3 * i + 1) / 2);
    }
    vector<ll> partition(n + 2);
    partition[0] = 1;
    for(i=1;i<=n+1;i++) {
        for(j=0; pentagon[j] <= i && j<pentagon.size(); j++) {
            if(j % 4 == 0 || j % 4 == 1) {
                partition[i] += partition[i - pentagon[j]];
            }
            else {
                partition[i] -= partition[i - pentagon[j]];
            }
            if(partition[i] >= mod) partition[i] -= mod;
            if(partition[i] < 0) partition[i] += mod;
        }
    }
    cout << partition[n+1]-1 << "\n";
