...
void solve(ll testcase){
    ...
    vector<ll>arr(n);
    ll sn=sqrt(n);
    vector<vector<ll>>pcnt(n/sn+1);
    for(i=0;i<n;i++){
        cin>>arr[i];
        pcnt[i/sn].push_back(arr[i]);
    }
    for(auto&k:pcnt)sort(all(k));
    cin>>m;
    for(j=0;j<m;j++){
        ll a,b,c,d;
        cin>>a>>b>>c;
...
//            debug<<st<<' '<<en<<' '<<ans<<'\n';
            if(st==en){
                ...
                for(i=st+1;i<en;i++){
                    ans+=pcnt[i].end()-upper_bound(pcnt[i].begin(), pcnt[i].end(),d);
                }
                for(i=b;i<(st+1)*sn;i++){
                    ans+=(arr[i]>d?1:0);
                }
            ...
            b--;
            *find(pcnt[b / sn].begin(), pcnt[b / sn].end(),arr[b])=c;
            arr[b]=c;
            sort(all(pcnt[b/sn]));
        }
    }
