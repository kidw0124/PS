...
void solve(){
    ...
    vector<ll>dp(n+m+1),ridx(n+m+1);
    vector<vector<ll>>sum(3,vector<ll>(n+m+1)),cnt(3,vector<ll>(n+m+1)),las(3,vector<ll>(n+m+1));
    vector<vector<ll>>pre(3,vector<ll>(n+m+1));
    for(i=1;i<=n+m;i++){
        cnt[1][i]=cnt[1][i-1];
        cnt[2][i]=cnt[2][i-1];
        sum[1][i]=sum[1][i-1];
