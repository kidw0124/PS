...
void solve(ll testcase) {
    ...
    ll n, m, r, q;
    cin>>n>>m;
    vector<pll>arr(n),brr(m);
    map<ll,multiset<ll>>crr,drr;
    set<ll>times_set,ts_set;
    vector<ll>times,ts;
    for(i=0;i<n;i++){
        cin>>arr[i].first>>arr[i].second;
        ts_set.insert(arr[i].second);
        drr[arr[i].second].insert(arr[i].first);
    }
    for(i=0;i<m;i++){
        cin>>brr[i].first>>brr[i].second;
        times_set.insert(brr[i].second);
        crr[brr[i].second].insert(brr[i].first);
    }
    for(auto k:times_set)times.push_back(k);
    for(auto k:ts_set)ts.push_back(k);
    ll cnt=0;
    ll en = times_set.size()-1;
    ordered_multiset OS;
    for(i=ts_set.size()-1;i>=0;i--){
        while(en>=0&&times[en]>ts[i]){
            for(auto k:crr[times[en]]){
                ll c2=OS.order_of_key(k+1);
                if(c2){
                    cnt++;
                    OS.erase(OS.find_by_order(c2-1));
                }
            }
            en--;
        }
