...
void solve(ll testcase) {
    ...
    ll maxp=min(m+1,n/3+1);
    vector<vector<ll>>dp(n,vector<ll>(maxp));
    for(i=0;i<n;i++) {
        for(j=0;j<n;j++) {
            cin >> arr[i][j];
            if(j>=2){
    ...
    for(i=0;i<n;i++){
        ...
        for(j=3;j<n;++j){
            ll p=j%3;
            ll q=(j+2)%3;
            for(k=min(m,(j+1)/3);k;--k){
                tmp[p][k]=max(tmp[q][k],tmp[p][k-1]+sum[i][j]);
            }
        }
        for(j=0;j<maxp;j++){
            dp[i][j]=max({tmp[0][j],tmp[1][j],tmp[2][j]});
        }
    }
    vector<ll>idx(n);
    priority_queue<pll>pq;
    for(i=0;i<n;i++){
        pq.push({dp[i][1]-dp[i][0],i});
        idx[i]=1;
    }
    ll ans=0;
    while(!pq.empty()&&m&&pq.top().first>0){
        pll p=pq.top();
        pq.pop();
        ans+=p.first;
        m--;
        if(idx[p.second]+1<maxp && dp[p.second][idx[p.second]+1]-dp[p.second][idx[p.second]]>0){
            pq.push({dp[p.second][idx[p.second]+1]-dp[p.second][idx[p.second]],p.second});
            idx[p.second]++;
        }
    }
    end=clock();
