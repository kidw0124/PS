...
int gcd2(int a,int b){
    ...
    if(a==0) return g[a][b]=b;
    if(b==0) return g[a][b]=a;
    return g[a][b]=gcd2(b,a%b);
};
struct rational{
    int a,b;
    bool operator<(const rational &o) const{
        if(b==0) return false;
        else if(o.b==0) return true;
        return a*o.b<o.a*b;
    }
};
...
void solve(ll testcase) {
    ...
    for(int i=1;i+i<=n;i++){
        for(int j=i+i;j<=n;j++){
            if(gcd2(j,i)==1){
                arr[cnt++]={i,j};
                if(i+i!=j) arr[cnt++]={j-i,j};
            }
        }
    }
    nth_element(arr.begin(),arr.begin()+m-1,arr.end());
//    debug<<clock()-start<<'\n'<<cnt<<'\n';
    cout<<arr[m-1].a<<" "<<arr[m-1].b;
