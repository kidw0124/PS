...
struct MaxFlowDinic{
    ...
    flow_t solve(int source, int sink) {
        ...
        while(assign_level(source, sink)) {
            ...
            while(flow_t flow = block_flow(source, sink, numeric_limits<flow_t>::max())) ans += flow;
        }
        return ans;
    }
};
...
void solve(ll testcase) {
    ...
    for(i=0;i<n;i++){
        ...
        for(j=0;j<x;j++){
            ll y;
            cin>>y;
            mfd.add_edge(i+2, n+y+1, 1);
        }
    }
    for(i=0;i<n;i++){
        mfd.add_edge(source, i+2, 1);
        mfd.add_edge(dou, i+2, 1);
    }
    for(i=0;i<m;i++){
        mfd.add_edge(n+i+2, sink, 1);
    }
    mfd.add_edge(source, dou, r);
    cout<<mfd.solve(source, sink);
