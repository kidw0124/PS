...
void solve(ll tc){
    ...
    int source=n*n+2*n,sink=n*n+2*n+1;
    Dinic dinic;
    dinic.init(n*n+2*n+2);
    for(i=0;i<n;i++){
        for(j=0;j<n;j++){
            cin>>arr[i][j];
            if(arr[i][j]>=2){
                dinic.add_edge(source,i*n+j,arr[i][j]-1);
                dinic.add_edge(i*n+j,n*n+i,arr[i][j]-1);
                dinic.add_edge(i*n+j,n*n+n+j,arr[i][j]-1);
            }
            else if(!arr[i][j]){
                dinic.add_edge(i*n+j,sink,1);
                dinic.add_edge(n*n+i,i*n+j,1);
                dinic.add_edge(n*n+n+j,i*n+j,1);
                cnt++;
            }
        }
    }
