...
template <const ll &MOD=mod> struct Matrix{
    ...
    Matrix operator*(const Matrix &a){
        ll i,j,k;
        Matrix ret(mat.size(),a.mat[0].size());
        for(i=0;i<mat.size();i++){
            for(j=0;j<a.mat[0].size();j++){
                for(k=0;k<mat[0].size();k++){
    ...
    Matrix inv(){
        ...
        for(i=0;i<mat.size();i++){
            for(j=i;j<mat.size();j++){
                if(mat[j][i]){
                    for(k=0;k<mat.size();k++){
                        ...
                        swap(ret.mat[i][k],ret.mat[j][k]);
                    }
                    break;
                }
            }
            ll inv=modinverse(mat[i][i],MOD);
            ...
            for(j=0;j<mat.size();j++){
                ...
                ll mul=mat[j][i];
                for(k=0;k<mat.size();k++){
                    mat[j][k]-=mat[i][k]*mul%MOD;
                    if(mat[j][k]<0) mat[j][k]+=MOD;
                    ret.mat[j][k]-=ret.mat[i][k]*mul%MOD;
                    if(ret.mat[j][k]<0) ret.mat[j][k]+=MOD;
                }
            }
        }
        return ret;
...
void solve(ll testcase){
    ...
    ll p,q,n,m;
    cin >> p>>q>>n>>m;
    Matrix mat(n+1,n+1),vec(n+1,1);
    vec[n][0]=1;
    for(i=0;i<=n;i++){
        mat[i][i]=1;
    }
    for(i=1;i<n;i++){
        mat[i][i-1]=-q*modinverse(p,mod)%mod;
        mat[i][i+1]=-(p-q)*modinverse(p,mod)%mod;
        if(mat[i][i-1]<0) mat[i][i-1]+=mod;
        if(mat[i][i+1]<0) mat[i][i+1]+=mod;
    }
    Matrix ans=mat.inv()*vec;
    cout << ans[m][0] << endl;
}
