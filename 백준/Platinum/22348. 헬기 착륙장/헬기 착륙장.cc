...
struct SegmentTree{
    ...
    ll rst(ll node, ll l, ll r){
        if(l==r){
            return seg[node]=dat[l];
        }
        else{
            ll mid=(l+r)>>1;
            return seg[node] = rst(node<<1,l,mid) + rst(node<<1|1,mid+1,r);
        }
    }
    ll query(ll node, ll l, ll r, ll s, ll e){
        if(s<=l && r<=e){
            ...
            ll x=0;
            if(mid>=s){
                x+= query(node<<1,l,mid,s,e);
            }
            if(mid<=e){
                x+= query(node<<1|1,mid+1,r,s,e);
            }
            return x;
        }
    }
    void update(ll node, ll l, ll r, ll idx, ll delta){
        if(idx<l || idx>r){
            return;
        }
        else if(l==r){
            ...
            seg[node]+=delta;
            ll mid=(l+r)>>1;
            update(node<<1,l,mid,idx,delta);
            update(node<<1|1,mid+1,r,idx,delta);
        }
    }
};
...
void preprocess(){
    ll i,j;
    ll MAXN = 50'000;
    ll sMAXN = sqrt((ld)MAXN+1);
    dp.resize(sMAXN+1,vector<ll>(MAXN+1));
    pfx.resize(sMAXN+1,vector<ll>(MAXN+2));
    dp[1][0]=1;
    dp[1][1]=1;
    for(i=2;i<=sMAXN;i++){
        for(j=0;j<=MAXN;j++){
    ...
    for(i=0;i<=sMAXN;i++){
        for(j=0;j<=MAXN;j++){
            pfx[i][j+1]=pfx[i][j]+dp[i][j];
            if(pfx[i][j+1]>=MOD)pfx[i][j+1]-=MOD;
        }
    }
}

ll ans(ll a, ll b, ll idx){
    if(a>b)return 0;
    a=max(0ll,a);
    return (pfx[idx][b+1]+MOD-pfx[idx][a])%MOD;
}
