...
vector<ll> func(vector<ll>arr){
    ...
    ll i,j,k,x;
    for(x=2;x<=n;x++){
        ll idx= find(all(arr),x)-arr.begin();
        if(idx>0 && arr[idx-1]==x-1) continue;
        if(idx>0){
            ret.push_back(idx-1);
            vector<ll> tmp(n);
            ll t=0;
            for(i=idx;i<n;i++) tmp[t++]=arr[i];
            tmp[t++]=arr[idx-1];
            for(i=0;i<idx-1;i++) tmp[t++]=arr[i];
            arr=tmp;
        }
        idx= find(all(arr),x-1)-arr.begin();
        if(idx<n-1){
            ret.push_back(idx+1);
            vector<ll> tmp(n);
            ll t=0;
            for(i=idx+2;i<n;i++) tmp[t++]=arr[i];
            tmp[t++]=arr[idx+1];
            for(i=0;i<=idx;i++) tmp[t++]=arr[i];
            arr=tmp;
        }
        idx= find(all(arr),x)-arr.begin();
        ret.push_back(idx);
        vector<ll> tmp(n);
        ll t=0;
        for(i=idx+1;i<n;i++) tmp[t++]=arr[i];
        tmp[t++]=arr[idx];
        for(i=0;i<idx;i++) tmp[t++]=arr[i];
        arr=tmp;
    }
...
void solve(ll testcase) {
    ...
    each(x,arr) cin>>x;
    each(x,brr) cin>>x;
    vector<ll> a_ans=func(arr);
    vector<ll> b_ans=func(brr);
    if((a_ans.size() & 1) != (b_ans.size() & 1)){
        if(a_ans.size()>b_ans.size()){
            if(m & 1){
                for(i=0;i<m;i++){
                    b_ans.push_back(0);
                }
    ...
    cout<<a_ans.size()<<'\n';
    for(i=0;i<a_ans.size();i++){
        cout<<a_ans[i]+1<<' '<<b_ans[i]+1<<'\n';
    }
