...
struct mergesort_tree{
    ...
    vector<ll>arr;
    mergesort_tree(ll n){
        tree.resize(LGTSIZE+1,vector<ll>(n));
        arr.resize(n);
    }
    void init(ll dep,ll l, ll r){
        if(l==r){
            tree[dep][l]=arr[l];
            return;
        }
        ...
        while(i<=m)tree[dep][k++]=tree[dep+1][i++];
        while(j<=r)tree[dep][k++]=tree[dep+1][j++];
    }
    ll query_bigger_than_k(ll dep,ll l,ll r,ll s,ll e,ll k){
        if(r<s || e<l)return 0;
        if(s<=l && r<=e){
            return r-l+1-(upper_bound(tree[dep].begin()+l,tree[dep].begin()+r+1,k)-tree[dep].begin()-l);
        }
        ll m = (l+r)>>1;
        return query_bigger_than_k(dep+1,l,m,s,e,k)+query_bigger_than_k(dep+1,m+1,r,s,e,k);
    }
    ll query_smaller_than_k(ll dep,ll l,ll r,ll s,ll e,ll k){
        if(r<s || e<l)return 0;
        if(s<=l && r<=e){
            return lower_bound(tree[dep].begin()+l,tree[dep].begin()+r+1,k)-tree[dep].begin()-l;
        }
        ll m = (l+r)>>1;
        return query_smaller_than_k(dep+1,l,m,s,e,k)+query_smaller_than_k(dep+1,m+1,r,s,e,k);
    }
    ll query_equal_to_k(ll dep,ll l,ll r,ll s,ll e,ll k){
        if(r<s || e<l)return 0;
        if(s<=l && r<=e){
            return upper_bound(tree[dep].begin()+l,tree[dep].begin()+r+1,k)-lower_bound(tree[dep].begin()+l,tree[dep].begin()+r+1,k);
        }
        ll m = (l+r)>>1;
        return query_equal_to_k(dep+1,l,m,s,e,k)+query_equal_to_k(dep+1,m+1,r,s,e,k);
    }
};
...
void solve(ll testcase){
    ...
    for(i=0;i<m;i++){
        ...
        a--;
        b--;
        ans = mt.query_bigger_than_k(0,0,n-1,a,b,c);
        cout<<ans<<endl;
    }
}
