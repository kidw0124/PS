...
void preprocess() {
    ll i,j;
    second_sterling[0][0] = 1;
    for(i=1;i<MAXN;i++) {
        for(j=1;j<=i;j++) {
            second_sterling[i][j] = (second_sterling[i-1][j-1] + j*second_sterling[i-1][j])%mod;
        }
    }
    factorial[0] = 1;
    for(i=1;i<MAXN;i++) {
    ...
    for(i=0;i<MAXN;i++) {
        for(j=0;j<=i;j++) {
            arr[i]+=(second_sterling[i][j]*factorial[j])%mod;
            arr[i]%=mod;
        }
    }
    for(i=0;i<MAXN;i++) {
...
void solve(ll testcase) {
    ...
    ll lim=n-m+1;
    for(i=1;i<=lim;i++) {
        ans[n][m] += comb[lim-1][i-1] * arr[lim - i] % mod;
        ans[n][m] %= mod;
    }
    ans[n][m]*=comb[n-1][lim-1];
    ans[n][m]%=mod;
    ans[n][m]*=arr[m-1];
    ans[n][m]%=mod;
    cout << ans[n][m] << '\n';
