...
void solve(ll testcase) {
    ...
    vector<vector<pll>> grp(n);

    vector<ll>par_cost(n);
    ...
    for(i=0;i<n-1;i++) {
        ...
        grp[a].push_back({b,arr[a]^arr[b]});

        grp[b].push_back({a,arr[a]^arr[b]});
    ...
    while(!q.empty()){
        ...
        for(auto x:child[u]){
            ...
            ans[x.first]=ans[u]+(n-2*total[x.first])*x.second;
