...
ll sum(ll n, ll m){
    ...
    if(m==0){
        ...
        vector<ll>arr(m+2),brr(m+2);
        arr[0]=1;
        brr[m+1]=1;
        for(i=0;i<m+1;i++){
            arr[i+1]=arr[i]*((n-i)%mod)%mod* modinverse(i+1,mod)%mod;
        }
        for(i=m+1;i>0;i--){
            brr[i-1]=brr[i]*((n-i)%mod)%mod* modinverse(m-i+2,mod)%mod;
        }
        vector<ll>dp(m+2);
        dp[0]=0;
        for(i=1;i<=m+1;i++){
        ...
        for(i=0;i<=m+1;i++){
            if((m-i+1)&1)ret+=mod-dp[i]*arr[i]%mod*brr[i]%mod;
            else ret+=dp[i]*arr[i]%mod*brr[i]%mod;
...
ll mpsum(ll m,ll n){ // sigma[1:m] x^n
	m%=M;
	ll res=0;
	if(n+5>m){
		for(ll i=1;i<=m;i++)
...
void solve(ll testcase){
    ...
    cin>>m>>r;
    n=1;
    ll ans=mpsum(m,r)+mod-mpsum(n-1,r);
