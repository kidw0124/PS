...
void solve(ll testcase){
    ...
    ll sn=sqrt(n);
    sort(qu.begin(), qu.end(),[&](const query&l, const query&r){
        return l.s/sn==r.s/sn?l.e<r.e:l.s<r.s;
    });
    ll st=qu[0].s,en=qu[0].e;
    ll ans=0;
    for(i=st;i<=en;i++){
        ans+=cnt.query(arr[i]+1,m);
        cnt.update(arr[i],1);
    }
    qu[0].ans=ans;
    for(i=1;i<qu.size();i++){
        while(en<qu[i].e){
            ll idx=++en;
            ans+=cnt.query(arr[idx]+1,m);
            cnt.update(arr[idx],1);
        }
        while(st>qu[i].s){
            ll idx=--st;
            ans+=cnt.query(1,arr[idx]-1);
            cnt.update(arr[idx],1);
        }
        while(en>qu[i].e){
            ll idx=en--;
