...
const ll maxn = 4e6+1;
int ans[maxn];
void mult(bitset<(int)1e6+1>&a, ll n, bitset<(int)1e6+1>&b, ll m) {
    ll fn = 1;
    while (fn < n + m) fn <<= 1; // n + m: interested length
    static float ra[maxn], rb[maxn], ia[maxn], ib[maxn];
    ...
    fft(1, fn, rb, ib);
    for (ll i = 0; i < fn; ++i) {
        float real = ra[i] * rb[i] - ia[i] * ib[i];
        float imag = ra[i] * ib[i] + rb[i] * ia[i];
        ra[i] = real, ia[i] = imag;
    }
    fft(-1, fn, ra, ia);
...
int main() {
    ...
    ll i,j;
    ll n;
    bitset<(int)1e6+1> primes,primes2;
    primes.set();
    primes[0]=primes[1]=0;
    for(i=2;i<=1e6;i++){
        if(primes[i]){
            if(i<=5e5)primes2[i<<1]=1;
            for(j=i+i;j<=1e6;j+=i){
                primes[j]=0;
            }
        }
    }
    mult(primes,1e6+1,primes2,1e6+1);
    ll t;
    cin>>t;
    while(t--){
        ll n;
