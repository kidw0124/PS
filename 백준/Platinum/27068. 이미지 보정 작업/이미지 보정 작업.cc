...
#else
const ll TSIZE = 1 << 18; // always 2^k form && n <= TSIZE
#endif
...
void solve(ll testcase){
    ...
    while(lo<hi){
        ll mid = (lo+hi)>>1;
        vector<vector<ll>>grp(n, vector<ll>(m, 0));
        for(i=0;i<n;i++){
            for(j=0;j<m;j++){
                grp[i][j] = arr[i][j]<x-mid;
            }
        }
        vector<vector<ll>>island(n, vector<ll>(m, 0));
        ll now = 0;
        ll cnt=0;
        for(i=0;i<n;i++){
            for(j=0;j<m;j++){
                if(grp[i][j] && !island[i][j]){
                    island[i][j] = ++now;
                    ll nowcnt = 1;
                    bool issum = 0;
                    queue<pair<ll,ll>>q;
                    ...
                    while(!q.empty()){
                        ...
                        q.pop();
                        for(int k=0;k<4;k++){
                            ll ni = a+dx[k], nj = b+dy[k];
                            if(ni>=0 && ni<n && nj>=0 && nj<m && grp[ni][nj] && !island[ni][nj]){
                                if(arr[ni][nj]-arr[a][b]>mid){
                                    issum = 1;
                                }
                                island[ni][nj] = now;
                                nowcnt++;
                            ...
                            else if(ni>=0 && ni<n && nj>=0 && nj<m && !grp[ni][nj]){
                                if(arr[ni][nj]-arr[a][b]>mid){
                                    issum = 1;
                                }
                            }
