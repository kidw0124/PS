...
void solve(ll testcase){
    ...
    priority_queue<pll, vector<pll>
            , decltype([](pll a, pll b) {
                return a.first+a.second>b.first+b.second;
            })>pq;
    for(i=0;i<m;i++){
        for(j=i;j<m;j++){
            if(arr[i].first+arr[j].first>=r) {
                pq.push({arr[i].first, arr[j].first});
            }
        }
        if(arr[i].first>=r)pq.push({arr[i].first,0});
    }
    ll ans=0;
    while(!pq.empty() && n){
        auto [a,b]=pq.top();
