...
void solve(ll testcase){
    ...
    ll a,b,c;
    cin>>n>>a>>b>>c;
    if(b&1||n<a+b+c)cout<<"0\n";
    else{
        ll ans=((facto[b]*facto[a])%mod*facto[c])%mod;
        auto ncr=[&](ll n, ll r){
            if(n<r||r<0)return 0ll;
            else return (facto[n]*invfacto[r])%mod*invfacto[n-r]%mod;
        };
        b>>=1;
        ans=(ans*ncr(b+c-1,c))%mod;
        ans=(ans*ncr(n-(b+c)-(a+b-1),a+b))%mod;
        ans=(ans*ncr(a+b,a))%mod;
        cout<<ans<<'\n';
    }
...
void preprocess() {
    ll i, j, k;
    facto.resize(MAXN+1),invfacto.resize(MAXN+1);
    facto[0]=1;
    for(i=1;i<=MAXN;i++){
        facto[i]=(facto[i-1]*i)%mod;
    }
    invfacto[MAXN]=modinverse(facto[MAXN],mod);
    for(i=MAXN-1;i>=0;i--){
        invfacto[i]=((i+1)*invfacto[i+1])%mod;
    }
//    debug<<invfacto[0]<<' '<<invfacto[1]<<' '<<invfacto[2]<<' '<<invfacto[3]<<'\n';
}
