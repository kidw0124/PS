...
void solve(ll testcase){
    ...
    vector<ll>arr(n+2);
    ll sum=0;
    for(i=1;i<=n;i++)cin>>arr[i],sum+=arr[i];
    ll ans=-2*sum;
    pll ansv={2,1};
    if(n==1)ans=0,ansv={1,1};
    vector<ll>pfx(n+2),sfx(n+2);
    for(i=1;i<=n;i++){
        pfx[i]=pfx[i-1]-2*arr[i];
    }
    for(i=n;i>=0;i--){
        sfx[i]=sfx[i+1]-2*arr[i];
    }
    ll now=1;
    set<pll>st;
    for(i=1;i<=n;i++){
        while(now-i<=m&&now<=n){
            st.insert({-sfx[now+1],now});
            now++;
        }
        ...
        else if(ans==pfx[i-1]+val){
            ansv=min(ansv,{i,idx});
        }
        st.erase({-sfx[i+1],i});
    }
    cout<<sum+ans<<'\n';
    cout<<ansv.first<<' '<<ansv.second<<'\n';
