...
void solve(ll testcase) {
    ...
    ll n,m;
    cin>>n>>m;
    vector<ll>facto(n<<1|1,1),invfacto(n<<1|1,1),inv(n<<1|1,1);
    for(i=1;i<=n<<1;i++) {
        facto[i] = facto[i-1]*i%mod;
    }
    invfacto[n<<1] = modinverse(facto[n<<1],mod);
    inv[n<<1] = invfacto[n<<1]*facto[(n<<1)-1]%mod;
    for(i=n+n-1;i>=0;i--) {
        invfacto[i] = invfacto[i+1]*(i+1)%mod;
        inv[i] = invfacto[i]*facto[i-1]%mod;
    }
    ll ans=modpow(n,m,mod);
    for(i=1;i<n;i++){
        ll tmp=modpow(n-i,m,mod);
        tmp*=facto[n<<1]*invfacto[i-1]%mod*invfacto[(n<<1)-i]%mod*(inv[i]+mod-inv[(n<<1)-i+1])%mod;
//        debugv(tmp);
        tmp%=mod;
        ans+=tmp;
        ans%=mod;
