...
pll operator*(const pll&l, const pll&r){
    ll p=l.first*r.first;
    if(l==pll(0,0)||r==pll(0,0))return pll(0,0);
    return {p%100000,(l.second||r.second||(p>100000))};
}
pll operator*(const ll&l, const pll&r){
    ll p=l*r.first;
    if(r==pll(0,0))return pll(0,0);
    return {p%100000,(r.second||(p>100000))};
}
pll operator+(const pll&l, const pll&r){
    ll p=l.first+r.first;
    return {p%100000,(l.second||r.second||(p>100000))};
}
pll operator+=(pll&l, const pll&r){
    return l=l+r;
}

pll func(ll st, ll en){
//    if(st==1&&en==5){
//        debug<<"1\n";
//    }
    ll x=0;
    ...
    else if(st==en)return {1,0};
    else if(mp.find({st,en})==mp.end()){
        pll ret = {0,0};
        ll i;
        if(str[st]!='('&&str[st]!='{'&&str[st]!='['&&str[st]!='?')return {0,0};
        for(i=st+1;i<en;i+=2){
            if(str[st]!='?'&&str[i]=='?'){
                ret+=func(st+1,i)*func(i+1,en);
            }
            else{
                switch (str[st]) {
                    case '(':
...
//    ll n,m,r;
    ...
    pll ans=func(0,n);
    if(ans.second){
        cout<<setw(5)<<setfill('0')<<ans.first;
    }
    else cout<<ans.first;
}
