...
ll ncr(ll n, ll r, ll MOD,vector<ll>&facto,vector<ll>&ifacto){
    return facto[n]*ifacto[r]%MOD*ifacto[n-r]%MOD;
}
...
void solve(ll testcase){
    ...
    cin>>n;
    vector<ll>facto(2*n+1);
    vector<ll>ifacto(2*n+1);
    facto[0]=1;
    for(i=1;i<=2*n;i++)facto[i]=facto[i-1]*i%MOD;
    ifacto[2*n]= modinverse(facto[2*n],MOD);
    for(i=2*n-1;i>=0;i--){
    ...
    for(i=0;i<n;i++){
        cin>>j;
        if(j==1){
            ans+=ncr(2*i,i,MOD,facto,ifacto)
                    *ncr(2*(n-1-i),n-1-i,MOD,facto,ifacto)%MOD;
            if(ans>=MOD)ans-=MOD;
        }
    }
    ans*=facto[n-1]*modinverse(modpow(2,n-1,MOD),MOD)%MOD;
    ans%=MOD;
    cout<<ans;
