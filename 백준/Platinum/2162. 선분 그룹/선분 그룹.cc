...
ll n;
pair<pii,pii>arr[3005];
ll ccw (pii a, pii b, pii c){
    return ((b.first-a.first)*(c.second-a.second)-(c.first-a.first)*(b.second-a.second));
}
ll intersect(pair<pii,pii>x, pair<pii,pii>y){
    pii x_a=x.first,x_b=x.second,y_a=y.first,y_b=y.second;
...
ll group[3005],group_cnt,goup_each[3005];
...
int main(){
    ...
    sort(goup_each,goup_each+n);
    reverse(goup_each,goup_each+n);
    for(i=0;i<n;i++){
        if(goup_each[i])group_cnt++;
    }
    cout<<group_cnt<<'\n'<<goup_each[0];
    return 0;
}
