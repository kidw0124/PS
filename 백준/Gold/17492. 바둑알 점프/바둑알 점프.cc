...
void func(){
    if(twos.size()==1){
        ...
        for(auto[x,y]:ntwos){
            for(ll i=0;i<8;i++) {
                auto [nx, ny] = pll(x + dx[i],y+dy[i]);
                auto [nnx, nny] = pll(nx + dx[i],ny+dy[i]);
                if(twos.find({nx,ny})!=twos.end()
                    && ones.find({nnx,nny})!=ones.end()){
                    twos.erase({x,y});
                    twos.erase({nx,ny});
                    ones.erase({nnx,nny});
                    ones.insert({x,y});
...
void solve(ll testcase) {
    ...
    for(i=0;i<n;i++){
        for(j=0;j<n;j++){
            cin>>m;
            if(m==0)ones.insert({i,j});
            if(m==2)twos.insert({i,j});
        }
    }
    func();
    cout<<"Impossible\n";
