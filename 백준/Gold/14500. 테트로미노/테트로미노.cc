...
vector<ll>dx({1,0,0,-1}),dy({0,1,-1,0});
...
ll func(ll x, ll y,ll now){
    if(now==4)return arr[x][y];
    else{
        for(ll i=0;i<4;i++){
            pos[x+dx[i]][y+dy[i]]++;
        }
        ...
        for(ll i=-1;i<=1;i++){
            for(ll j=-1;j<=1;j++) {
                if (!vis[x + i][y + j] && pos[x + i][y + j]) {
                    vis[x + i][y + j] = 1;
                    p = max(p, func(x + i,y+j, now + 1));
                    vis[x + i][y + j] = 0;
                }
            }
        }
...
void solve(){
    ll i,j;
    cin>>n>>m;
    arr.resize(n+10,vector<ll>(m+10));
    pos.resize(n+10,vector<ll>(m+10));
    vis.resize(n+10,vector<bool>(m+10,1));
    for(i=0;i<n;i++){
        for(j=0;j<m;j++){
            cin>>arr[i+5][j+5];
            vis[i+5][j+5]=0;
