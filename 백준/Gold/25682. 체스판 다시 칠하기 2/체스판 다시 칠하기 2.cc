...
void solve(){
    ...
    cin>>n>>m>>k;
    vector<string>str(n);
    vector<vector<vector<ll>>>sum(2,vector<vector<ll>>(n+1,vector<ll>(m+1)));
    for(i=0;i<n;i++){
        cin>>str[i];
    }
    for(i=0;i<n;i++){
        for(j=0;j<m;j++){
            sum[(i+j)&1][i+1][j+1]=sum[(i+j)&1][i+1][j]+sum[(i+j)&1][i][j+1]-sum[(i+j)&1][i][j]+(str[i][j]=='B');
            sum[(i+j)&1^1][i+1][j+1]=sum[(i+j)&1^1][i+1][j]+sum[(i+j)&1^1][i][j+1]-sum[(i+j)&1^1][i][j];
        }
    }
    ll ans=k*k;
    for(i=0;i<=n-k;i++){
        for(j=0;j<=m-k;j++){
            ll x=sum[0][i+k][j+k]-sum[0][i][j+k]-sum[0][i+k][j]+sum[0][i][j];
            ll y=sum[1][i+k][j+k]-sum[1][i][j+k]-sum[1][i+k][j]+sum[1][i][j];
            y=k*k/2+((k&1)&(i+j))-y;
            ans=min({x+y,(k*k-x-y),ans});
        }
    }
    cout<<ans;
}
