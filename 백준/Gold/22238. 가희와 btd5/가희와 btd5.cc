...
void solve(ll testcase) {
    ll i, j, k;
    ll n, m, r,suma=0,sumb=0,sumc=0,sumd=0;
    cin>>n>>m;
    ll a,b,c,ga,gb;
    vector<ll>arr,brr,crr,drr;
    for(i=0;i<n;i++){
        cin>>a>>b>>c;
        ga=abs(a/gcd(a,b)),gb=abs(b/gcd(a,b));
        if(a>=0&&b>=0)arr.push_back(c);
        if(a<0&&b>=0)brr.push_back(c);
        if(a<0&&b<0)crr.push_back(c);
        if(a>=0&&b<0)drr.push_back(c);
    }
    sort(all(arr));
    sort(all(brr));
    sort(all(crr));
    ...
    for(i=0;i<m;i++){
        ll x,y,gx,gy;
        cin>>x>>y>>c;
        gx=abs(x/gcd(x,y)),gy=abs(y/gcd(x,y));
        debug<<gx<<' '<<gy<<' '<<ga<<' '<<gb<<'\n';
        if(ga==gx&&gb==gy){
            if(x>=0&&y>=0)suma+=c;
            if(x<0&&y>=0)sumb+=c;
        ...
        cout<<arr.end()-upper_bound(all(arr),suma)
              +brr.end()-upper_bound(all(brr),sumb)
              +crr.end()-upper_bound(all(crr),sumc)
              +drr.end()-upper_bound(all(drr),sumd)<<'\n';
    }
}
