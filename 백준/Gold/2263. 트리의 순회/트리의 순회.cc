...
vector<int>inord,postord,rev;

void func(int is, int ie, int st,int en){
    if(st>en){
        return;
    }
    int root=postord[en];
    ...
    if(st==en){
        ...
        int loc=rev[root];
        int lef=loc-is,rig=ie-loc;
        if(lef)func(is,is+lef-1,st,st+lef-1);
        if(rig) func(is+lef+1,ie,st+lef,en-1);
    }
}
...
void solve(){
    ...
    ll n;
    cin>>n;
    inord.resize(n),postord.resize(n),rev.resize(n+1);
    for(i=0;i<n;i++){
        cin>>inord[i];
        rev[inord[i]]=i;
    }
