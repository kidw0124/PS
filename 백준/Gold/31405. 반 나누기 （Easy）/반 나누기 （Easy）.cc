...
pld divide_line(pld a, pld b, ld alpha){
...
void solve(ll testcase){
    ...
    for(i=0;i<n;i++){
        ll j=(i+1)%n;
        sum[i]=tot+arr[i].first*arr[0].second-arr[i].second*arr[0].first;
        tot+=arr[i].first*arr[j].second-arr[i].second*arr[j].first;
    }
    ll idx= upper_bound(all(sum),tot/2)-sum.begin()-1;
    ll nxt=(idx+1)%n;
    ld st=0,ed=1;
    while(st+eps<ed){
        ld mid=(st+ed)/2;
        ld now=sum[idx]+(sum[nxt]-sum[idx])*mid;
        if(now>tot/2) ed=mid;
        else st=mid;
    }
    cout<<"YES\n1 0\n";
    cout<<idx+1<<fixed<<setprecision(12)<<' '<<st<<'\n';
}
