...
void solve(ll testcase){
    ...
    ld ans=dist(a,b);
    if(p&&q){
        pld ax={a.first,(r-p*a.first)/q};
        pld bx={b.first,(r-p*b.first)/q};
        pld ay={(r-q*a.second)/p,a.second};
        pld by={(r-q*b.second)/p,b.second};
        ans=min(ans,dist2(ax,bx)+dist(a,ax)+dist(b,bx));
        ans=min(ans,dist2(ay,bx)+dist(a,ay)+dist(b,bx));
        ans=min(ans,dist2(ax,by)+dist(a,ax)+dist(b,by));
        ans=min(ans,dist2(ay,by)+dist(a,ay)+dist(b,by));
    }
