...
int main() {
    ...
    for(i=0;i<n;i++){
        for(j=0;j<n;j++){
            if(i==j)continue;
//            i=upper_bound(all(arr),arr[i])-arr.begin()-1;
//            j=upper_bound(all(arr),arr[j])-arr.begin()-1;
//            if(i==j)i--;
            ll x = lower_bound(all(arr),a-arr[i]+1)-1-arr.begin(),y;
            while (x==j||x==i){
                x--;
            }
            y = lower_bound(all(arr),b-arr[j]+1)-1-arr.begin();
            while (y==x||y==i||y==j)y--;
            if(x<0||y<0)continue;
            else{
                ans=max(ans,arr[i]+arr[j]+arr[x]+arr[y]);
            }
        }
    }
