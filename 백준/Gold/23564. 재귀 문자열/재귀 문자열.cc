...
void preprocess() {
    ...
    auto check=[&](string str, ll x){
        ll i,j,k;
        string tn=str.substr(0,x-1);
        debug<<str<<" "<<x<<endl;
        char sn=str[x-1];
        for(i=0;i<str.size();i+=x){
            debug<<i<<" "<<str.substr(i,x-1)<<" "<<tn<<" "<<sn<<" "<<str[i+x]<<endl;
            if(tn!=str.substr(i,x-1)) return false;
            if(i+x-1<str.size() && sn!=str[i+x-1]) return false;
        }
        return true;
    };

    function<bool(string)> func=[&](string str){
        ll i,j,k;
        ll n=str.size()+1;
        if(n==1){
            return true;
        ...
        for(i=1;i*i<=n;i++){
            if(n%i==0){
                ...
                {
                    if(i>1 && check(str,n/i)){
                        ans.push_back(i);
                        ans_str+=str[n/i-1];
                        if(func(str.substr(0,n/i-1))){
                            return true;
                        }
                        ans_str.pop_back();
                        ans.pop_back();
