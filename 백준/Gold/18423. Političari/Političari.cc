...
void solve(ll testcase){
    ...
    rmp.push_back({0,1});
    pll pnow = {0,1};
    while(now<m && pnow.first<n){
        ll nex = arr[pnow.second][pnow.first];
        if(mp.find({pnow.second,nex})==mp.end()){
            mp[{pnow.second,nex}] = ++now;
            rmp.push_back({pnow.second,nex});
            pnow = {pnow.second,nex};
        }
        else{
            ll idx = mp[{pnow.second,nex}];
            ll len = now-idx+1;
            ll rem = m-idx;
            ll remm = rem%len;
            debug<<idx<<" "<<len<<" "<<rem<<" "<<remm<<endl;
            cout<<rmp[idx+remm].first+1;
            return;
        }
    }
    for(auto x:rmp){
        debug<<x.first<<" "<<x.second<<endl;
    }
    cout<<rmp[m].first+1;
