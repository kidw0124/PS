...
ld calc(ld a, ld b, ld c, ld d, ld e, ld x){
    return a*x*x*x+b*x*x+c*x+d+e;
}
...
void solve(ll testcase){
    ...
    while(en-st>eps){
        ...
        ld ret1=0, ret2=0;
        for(i=0;i<n;i++){
            ld val1 = (arr[i].second-arr[i].first*mid1);
            ld val2 = (arr[i].second-arr[i].first*mid2);
            ret1+=val1*val1*val1*val1;
            ret2+=val2*val2*val2*val2;
        }
        if(ret1<ret2) en=mid2;
        else st=mid1;
    }
