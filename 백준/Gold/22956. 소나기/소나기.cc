...
vector<vector<ll>>arr,vis;
vector<vector<pair<pll,pll>>>val;

pll _find(pll x){
    if(x==par[x.first][x.second])return x;
    else{
        pll p=par[x.first][x.second];
        if(val[p.first][p.second]>val[x.first][x.second])val[p.first][p.second]=val[x.first][x.second];
        return par[x.first][x.second]=_find(p);
    }
}

void _union(pll x, pll y){
    x= _find(x);
    y= _find(y);
    if(val[x.first][x.second]<val[y.first][y.second]){
        val[y.first][y.second]=val[x.first][x.second];
        par[y.first][y.second]=x;
    }
    else{
        val[x.first][x.second]=val[y.first][y.second];
...
void solve(ll testcase) {
    ...
    vis.resize(n,vector<ll>(m));
    par.resize(n,vector<pll>(m));
    val.resize(n,vector<pair<pll,pll>>(m));
    for(i=0;i<n;i++){
        for(j=0;j<m;j++){
            cin>>arr[i][j];
            par[i][j]={i,j};
            val[i][j]= {{arr[i][j], -1},{i,j}};
        }
    }
    for(i=0;i<q;i++){
        ll a,b,c;
        ...
        arr[a][b]-=c;
        vis[a][b]=1;
        val[a][b]=min(val[a][b],{{arr[a][b],i},{a,b}});
        for(j=0;j<4;j++){
            ll na=a+dx[j],nb=b+dy[j];
            if(na<0||nb<0||na>=n||nb>=m)continue;
            else if(vis[a][b]&&vis[na][nb]){
                _union({a,b},{na,nb});
            }
        }
        auto [px,py]=_find({a,b});
        cout<<val[px][py].second.first+1<<' '<<val[px][py].second.second+1<<'\n';
    }
}
