...
public:
    ...
    Matrix<T> POWsum(const ll&r,const ll&mod)const{
        if(r==1){
            return (*this)%mod;
        }
        else{
            Matrix<T> half=POWsum(r>>1,mod);
            if(r&1){
                return (half+POW(r>>1,mod)*half+POW(r,mod))%mod;
            }
            else return (half+POW(r>>1,mod)*half)%mod;
        }
    }
